...
# from fconv import FConv, FDWConv, FGhostConv, FLightConv, FRepConv, autopad
from fconv import FConv, FDWConv , FLightConv, autopad

# from ftransformer import TransformerBlock
...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = FConv(c1, 2 * self.c, 1, 1)
        self.cv2 = FConv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(
            Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0)
            for _ in range(n)
        )
    ...
    def set_weights(self, weights_dict: Dict[str, Union[torch.Tensor, dict]]):
        self.cv1.set_weights(weights_dict["cv1"])
        self.cv2.set_weights(weights_dict["cv2"])
        for i, m in enumerate(self.m):
            m.set_weights(weights_dict[f"m.{i}"])
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = FConv(c1, c_, k[0], 1)
        self.cv2 = FConv(c_, c2, k[1], 1, g=g)
        self.add = shortcut and c1 == c2
