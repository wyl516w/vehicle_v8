# Rewrite from Ultralytics YOLO 🚀, AGPL-3.0 license
"""Convolution modules."""
...
class FConv2d(nn.Module):
    def __init__(
        ...
        self.weight = torch.Tensor(out_channels, in_channels // groups, *kernel_size)
        self.bias = torch.Tensor(out_channels) if bias else None
    ...
    def set_weights(self, weights_dict: Dict[str, Union[torch.Tensor, dict]]):
        self.weight = weights_dict["weight"]
        if "bias" in weights_dict and self.bias is not None:
...
class FConv(nn.Module):
    ...
    def set_weights(self, weights_dict: Dict[str, Union[torch.Tensor, dict]]):
        self.conv.set_weights(weights_dict["conv"])
        if "bn" in weights_dict:
            self.bn.set_weights(weights_dict["bn"])
...
class FConv2(FConv):
    ...
    def set_weights(self, weights_dict: Dict[str, Union[torch.Tensor, dict]]):
        self.conv.set_weights(weights_dict["conv"])
        self.cv2.set_weights(weights_dict["cv2"])
        if "bn" in weights_dict:
            self.bn.set_weights(weights_dict["bn"])


class FLightConv(nn.Module):
    """
    ...
    def __init__(self, c1, c2, k=1, act=nn.ReLU()):
        """Initialize Conv layer with given arguments including activation."""
        super().__init__()
        self.conv1 = FConv(c1, c2, 1, act=False)
        self.conv2 = FDWConv(c2, c2, k, act=act)
    ...
    def set_weights(self, weights_dict: Dict[str, Union[torch.Tensor, dict]]):
        ...
        self.conv2.set_weights(weights_dict["conv2"])
